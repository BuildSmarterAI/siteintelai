...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price: "price_1SHcbzAsWVx52wY3noAgZVsf", // SiteIntel Pro Subscription
          quantity: 1,
        },
      ],
      mode: "subscription",
    ...
    console.error("Error in create-subscription:", error instanceof Error ? error.message : String(error));
    return new Response(JSON.stringify({ error: error instanceof Error ? error.message : "Subscription processing failed" }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 500,
    });
  }
