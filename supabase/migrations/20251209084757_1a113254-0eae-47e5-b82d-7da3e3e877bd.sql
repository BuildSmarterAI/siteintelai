-- Drop and recreate with proper EWKT handling
...
BEGIN
  ...
  LOOP
    ...
    -- Handle geometry column specially - use ST_GeomFromEWKT for EWKT format
    IF v_key = 'geom' AND v_value IS NOT NULL AND v_value != 'null'::jsonb THEN
      ...
      IF v_geom_value LIKE 'SRID=%' THEN
        v_values := array_append(v_values, format('ST_GeomFromEWKT(%L)', v_geom_value));
      ELSIF v_geom_value LIKE '{%' THEN
        v_values := array_append(v_values, format('ST_SetSRID(ST_GeomFromGeoJSON(%L), 4326)', v_geom_value));
      ELSE
