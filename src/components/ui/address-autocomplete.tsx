...
export function AddressAutocomplete({
  ...
  const handleSuggestionClick = async (suggestion: AddressSuggestion) => {
    try {
      ...
      // Trigger GIS enrichment if applicationId provided
      if (applicationId && coordinates) {
        ...
        try {
          ...
          if (enrichData?.success) {
            setEnrichmentStatus(enrichData.dataFlags?.length > 0 ? 'partial' : 'success');
    ...
    <div className="relative">
      ...
      {enrichmentStatus === 'partial' && (
        <div className="mt-2 flex items-center gap-2 text-sm text-yellow-600">
          ...
          <span>Partial data loaded - some fields require manual entry</span>
        </div>
      )}
