...
const float ASCII_AMT    = 0.18;  
const float ASCII_SCALE  = 30.0;  
...
// mesh-gradient: BuildSmarter brand colors
vec3 meshGradient(vec2 uv){
  vec2 p0=vec2(-0.70,-0.45), p1=vec2(0.75,-0.35), p2=vec2(-0.65,0.65), p3=vec2(0.80,0.55);
  vec3 c0=vec3(0.039,0.059,0.176);  // Midnight Blue #0A0F2C
  vec3 c1=vec3(0.067,0.137,0.310);  // Navy #11224F
  vec3 c2=vec3(0.031,0.055,0.157);  // Dark variant
  vec3 c3=vec3(0.024,0.376,0.765);  // Data Cyan #06B6D4
  float e=2.0;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  vec3 lineThin  = vec3(0.024,0.714,0.831);  // Data Cyan
  vec3 lineMajor = vec3(0.400,0.847,0.937);  // Lighter Cyan
