...
import { BuyPathTour } from "@/components/tours/BuyPathTour";
import { BuildPathTour } from "@/components/tours/BuildPathTour";
import { isTourCompleted } from "@/lib/tourStyles";
...
export function QuickCheckResult({ score, band, floodRisk, zoningVerdict, address, intentType }: QuickCheckResultProps) {
  ...
  // Show appropriate tour after results load
  useEffect(() => {
    const tourKey = intentType === 'buy' ? 'buy_path' : 'build_path';
    if (!isTourCompleted(tourKey)) {
      // Delay tour start to let animations complete
  ...
  return (
    <>
      {/* Tour Components */}
      {intentType === 'buy' ? (
        <BuyPathTour run={showTour} onFinish={() => setShowTour(false)} />
      ) : (
        <BuildPathTour run={showTour} onFinish={() => setShowTour(false)} />
      )}

      <div className="mt-6 space-y-4 animate-in fade-in-50 duration-500" data-tour="quickcheck-widget">
        {/* Score Display */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {intentType === 'build' ? (
            <>
              ...
              >
                <Card data-tour="utility-insight">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm flex items-center gap-2">
            ...
            <>
              <motion.div
                ...
                transition={{ delay: 0.3, duration: 0.3, ease: "easeOut" }}
              >
                <Card data-tour="market-value-insight">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm flex items-center gap-2">
              ...
              <motion.div
                ...
                transition={{ delay: 0.35, duration: 0.3, ease: "easeOut" }}
              >
                <Card data-tour="flood-insurance-insight">
                <CardHeader className="pb-3">
                  <CardTitle className="text-sm flex items-center gap-2">
        ...
        <div className="relative">
          ...
          <div className="absolute inset-0 flex items-center justify-center" data-tour="unlock-cta">
            <Button
              size="lg"
