...
export const Hero = () => {
  ...
  return <motion.section ref={sectionRef} className="relative flex min-h-screen overflow-hidden bg-gradient-to-br from-[#0A0F2C] via-[#11224F] to-[#0A0F2C]" variants={containerVariants} initial="hidden" animate="visible">
      ...
      {!isMobile ? <motion.div className="absolute inset-0 z-0" initial={{
      opacity: 0
    }} animate={{
      opacity: 0.18
    }} transition={{
      ...
      opacity: backgroundOpacity
          ...
          <picture className="absolute right-0 top-0 w-full h-full">
            <source srcSet={`${aerialPropertySite768webp} 768w, ${aerialPropertySite1024webp} 1024w, ${aerialPropertySite1920webp} 1920w`} sizes="100vw" type="image/webp" />
            <img src={aerialPropertySite1920jpg} alt="" loading="eager" decoding="async" className="w-full h-full object-cover blur-[2px] scale-105" />
          </picture>
          <div className="absolute inset-0 bg-gradient-to-r from-[#0A0F2C] via-transparent to-[#0A0F2C]/60" />
        </motion.div> : <div className="absolute inset-0 z-0 bg-gradient-to-br from-[#0A0F2C] via-[#11224F] to-[#0A0F2C]">
      ...
      y: gridY
        ...
        {!shouldDisableAnimations && isVisible && <div className="absolute inset-0 h-full w-[200%] bg-gradient-to-r from-transparent via-[#FF7A00]/15 to-transparent" style={{
        ...
        {!shouldDisableAnimations && isVisible && <svg className="absolute inset-0 w-full h-full pointer-events-none" aria-hidden="true" role="presentation">
            <defs>
          ...
          return <path key={`line-${i}`} d={`M ${startX}% ${startY}% Q ${(startX + endX) / 2}% ${(startY + endY) / 2 - 5}% ${endX}% ${endY}%`} stroke="url(#verificationGlow)" strokeWidth="1.5" fill="none" strokeDasharray="1000" strokeDashoffset="1000" style={{
        ...
        zIndex: 1
          ...
          x: 25,
          y: 85
        }].map((pos, i) => <motion.g key={`bracket-${i}`} initial={{
          opacity: 0
        }} animate={{
          opacity: 0.6
        }} transition={{
          ...
          delay: 1.5 + i * 0.1
              ...
              <path d={`M ${pos.x}% ${pos.y}% h 2.5 M ${pos.x}% ${pos.y}% v 2.5`} stroke="rgba(255, 255, 255, 0.6)" strokeWidth="2" fill="none" strokeLinecap="square" />
            </motion.g>)}
        </svg>
      </motion.div>

      {/* Step 3: Data Verification Nodes & Building Footprints - Reduced count */}
      {!shouldDisableAnimations && isVisible && <div className="absolute inset-0 pointer-events-none" aria-hidden="true" role="presentation">
          {[...Array(isMobile ? 3 : 4)].map((_, i) => {
        const x = 10 + i % 5 * 20;
        const y = 15 + Math.floor(i / 5) * 22;
        const delay = i * 0.5;
        return <motion.div key={`node-${i}`} className="absolute" style={{
          left: `${x}%`,
          top: `${y}%`,
          x: nodeX
        }}>
            ...
            ease: [0.45, 0, 0.2, 1]
                ...
                {/* Cyan node */}
                <motion.div className="absolute w-[6px] h-[6px] rounded-full bg-[#06B6D4] shadow-[0_0_10px_rgba(6,182,212,0.8)]" style={{
            transform: 'translate(-50%, -50%) translateZ(0)',
            contain: 'layout style paint'
          }} animate={{
            opacity: [0.4, 1, 0.4],
      ...
      <motion.div className="absolute right-0 top-0 h-full w-full lg:w-1/2 opacity-20" style={{
      y: prefersReducedMotion ? 0 : backgroundY
    }}>
        <div className="relative h-full w-full">
          {/* Step 4: Data Stream Visualization - Reduced count */}
          {!shouldDisableAnimations && isVisible && <svg className="absolute inset-0 w-full h-full pointer-events-none z-[4]" aria-hidden="true" role="presentation">
              <defs>
            ...
            return <g key={`stream-${i}`}>
                  <motion.line x1={`${startX}%`} y1={`${startY}%`} x2={`${endX}%`} y2={`${endY}%`} stroke="url(#streamGradient)" strokeWidth="1" initial={{
                pathLength: 0,
                opacity: 0
              }} animate={{
                pathLength: [0, 1, 0],
                ...
                ease: "easeInOut"
                    ...
                    <motion.foreignObject width="14" height="14" animate={{
                  x: [startX + '%', endX + '%'],
                  y: [startY + '%', endY + '%']
                }} transition={{
                  duration: 4,
          ...
          {!shouldDisableAnimations && isVisible && [...Array(3)].map((_, i) => <motion.div key={i} className="absolute h-2 w-2 rounded-full bg-[#06B6D4] shadow-[0_0_10px_rgba(6,182,212,0.5)]" style={{
        ...
        <div className="container relative mx-auto px-4 py-12 md:px-6 md:py-20 lg:py-24">
          ...
          boxShadow: '0 16px 48px rgba(10, 15, 44, 0.8), inset 0 0 60px rgba(6, 182, 212, 0.08)'
            ...
            <div className="mx-auto max-w-3xl text-center md:text-left relative z-20">
              ...
              {/* Headline */}
              <motion.h1 className="font-sans text-4xl md:text-5xl lg:text-6xl font-bold leading-[1.1] mb-6 md:mb-8 bg-gradient-to-r from-[#06B6D4] to-[#FF7A00] bg-clip-text text-transparent" variants={headlineVariants}>
